...
const AllBorrowReq = () => {
  ...
  // const handleSetPrice = async (borrowId) => {
  //   try {
  //     await setPrice(borrowId, BigInt(price));
      ...
      <div className="space-y-4">
        {borrowData.length > 0 ? (
          borrowData.map(borrow => (
            <div key={borrow.borrowId} className="flex space-x-12 w-[65rem] text-black py-4 m-2 text-lg">
              ...
              <button
                ...
                onClick={() => handleSetPrice(borrow.borrowId)}
              >
                Set Price
              </button>
              {/* <button
